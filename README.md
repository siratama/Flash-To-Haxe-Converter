...
###クラスフィールドプロパティへのアクセスは注意が必要

CreateJS-Haxe 用に出力された Haxe クラス内 フィールドプロパティは、Toolkit for CreateJS の仕様により undefined になってしまう場合があります。最善の注意を払うのであれば、TFCLib の ContainerUtil.getProperty メソッド経由でプロパティ取得を行います。

詳細は以下の URL 内「考えが抜けていた点と対処」の項をご確認ください。
